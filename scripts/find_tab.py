...
适配坑（普渡/卓驭 italent 系）：styled-components 导航"校园招聘"是 `div.sc-*`（无 role=tab），
选择器须含裸 `div` 才能命中；命中后若为下拉父菜单（点击展开二级"校招职位/实习生职位"），
由 hover_expand / 坐标点击二级菜单处理（见 site-notes 坑点6/7）。
"""
...
def find_tab(js, mode):
    ...
    (function() {
        ...
        for (let area of areas) {
            for (let el of area.querySelectorAll('a, button, li, span, div[role=tab], div')) {
