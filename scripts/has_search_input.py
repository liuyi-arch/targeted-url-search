...
顺序按 3 轮 23 站实测成功率排序：
- find_visible_search_input()—— 方法1（**推荐，实测 18/18=100% 主力**）：只返回**可见**搜索框（过滤 offsetParent!==null && rect.width>0）。
  适配坑：italent/zhiye 系（普渡/卓驭/360）与美团页面存在**两个同名"搜索职位关键词"输入框**，
  第一个隐藏（rect 0,0），find_search_input 会误命中它导致 fill 无效 → 必须走本方法定位可见项。
- find_clickable_search_input()—— 方法2（VIVO/t-ray 坑专用）：逐框 elementFromPoint 校验，返回**真正可点击**的搜索框。
- find_search_input()          —— 方法3：主文档定位（不保证可见性，隐藏框会误命中，仅作 fallback）。
- find_in_iframe()             —— 方法4：主文档找不到时 iframe 兜底定位（跨源 contentDocument 抛错则跳过）。
- has_search_input()           —— 3b 验证复用：仅判断页面是否出现搜索框（bool），非 3c 定位链成员。
"""


def find_visible_search_input(js):
    """方法1（推荐）：定位**可见**搜索框（offsetParent!==null && rect.width>0）。返回元素信息或 None。
...
def find_clickable_search_input(js):
    """方法2（VIVO 坑专用）：逐框 elementFromPoint 校验，返回**真正可点击**的搜索框（命中元素为 INPUT）。
...
def find_search_input(js):
    """方法3：定位搜索框（placeholder 含 搜索/职位/岗位/search/keyword）。返回 {id, name, placeholder} 或 None。
...
def find_in_iframe(js):
    """方法4：主文档无搜索框时，遍历 iframe 在子文档内定位。返回元素信息或 None（跨源抛错则跳过）。"""
    res = js("""
    (function() {
